...
type InvoiceStatusProps = { status: PaymentStatus }
const InvoiceStatus: React.FC<InvoiceStatusProps> = ({ status }) => {
	...
	return <div 
		className="list-item list-disc list-inside capitalize bg-[color:var(--bg)] text-[color:var(--color)] min-w-[104px] text-center font-bold bg-opacity-60 px-[18px] py-3 rounded-[6px]" 
		style={style}
	>
		{status}
	</div>
