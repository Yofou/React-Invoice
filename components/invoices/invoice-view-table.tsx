...
const InvoiceTableHeading: React.FC<{ className?: string }> = ({
	...
	return (
		<h1
			className={`hidden text-grey-300 text-body-2 sm:block ${className ?? ""}`}
		>
			{children}
		</h1>
	);
...
export const InvoiceViewTable: React.FC<InvoiceViewTableProps> = ({
	...
	return (
		<div
			className={`grid w-full grid-flow-row grid-cols-[max-content,1fr] justify-items-end rounded-t-[8px] bg-grey-1200 p-6 pb-10 sm:grid-cols-[minmax(286px,max-content),repeat(3,1fr)] sm:gap-y-8 sm:p-8 ${
				className ?? ""
			}`}
		>
			...
			{invoice?.items.map((item, index) => (
				<React.Fragment key={index}>
					<InvoiceTableItem className="mt-6 justify-self-start !text-white-full first-of-type:mt-0 sm:mt-0">
						{item.name}
					</InvoiceTableItem>
					...
					<h2 className="row-span-2 translate-y-[80%] transform self-center text-white-full text-h4 first-of-type:transform-none sm:row-auto sm:transform-none">
						{priceFormater.format(item.price * item.qty)}
					</h2>
					<p className="justify-self-start text-grey-600 text-h4 sm:hidden">
